...
<div class="row container">
    <?php
        ...
        //default sql-query
        $sql = 'SELECT id, post_content, post_title FROM ru_posts WHERE post_type = "post" AND post_content LIKE "%<img %" ORDER BY post_date DESC LIMIT 8';
        
        //if date was passed
        if ( $wp_query->query ) {
            if ( $posts_year && $posts_month && $posts_day ) {
                //change default sql-request
                $sql = 'SELECT id, post_content, post_title FROM ru_posts WHERE post_type = "post" AND ( ( YEAR( ru_posts.post_date ) =' . $posts_year . ' AND MONTH( ru_posts.post_date ) =' . $posts_month . ' AND DAYOFMONTH( ru_posts.post_date ) =' . $posts_day . ' ) ) AND post_content LIKE "%<img %" ORDER BY post_date DESC LIMIT 8';
            }
        }
        global $wpdb;
        ...
        <div class="row container all-news">
            <div class="col l9 m9 s12">';
                ...
                if ( $query->max_num_pages > 1 ) { ?>
                    ...
                    <a class="btn waves-effect waves-light" id="loadmore">Загрузить еще</a>
